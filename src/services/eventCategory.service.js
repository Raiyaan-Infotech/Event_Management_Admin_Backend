...
const MODEL_NAME = 'EventCategory';
const MODULE_SLUG = 'event_categories';
...
const getAll = async (query = {}, companyId = undefined) => {
    // Default to sort_order so the list matches idx_event_categories_listing;
    // an explicit sort_by in the query still wins.
    const listQuery = { sort_by: 'sort_order', sort_order: 'ASC', ...query };

    return baseService.getAll(EventCategory, MODEL_NAME, listQuery, {
...
const getById = async (id, companyId = undefined) => {
    return baseService.getById(EventCategory, MODEL_NAME, id, { companyId });
};
...
const create = async (data, userId = null, companyId = undefined) => {
    ...
    const nameExists = await EventCategory.findOne({
        where: { company_id: companyId ?? null, name: payload.name },
        attributes: ['id'],
    });
    if (nameExists) {
        throw ApiError.badRequest(`An event category named "${payload.name}" already exists.`);
    }

    return baseService.create(EventCategory, MODEL_NAME, payload, userId, companyId);
};

const update = async (id, data, userId = null, companyId = undefined) => {
    const category = await EventCategory.findByPk(id);
    if (!category) throw ApiError.notFound('Event category not found');
    ...
    return baseService.update(EventCategory, MODEL_NAME, id, payload, userId, companyId);
};
...
const deleteById = async (id, userId = null, companyId = undefined) => {
    return baseService.remove(EventCategory, MODEL_NAME, id, userId, companyId);
};
