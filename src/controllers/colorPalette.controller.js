...
const getAll = asyncHandler(async (req, res) => {
    const result = await colorPaletteService.getAll(req.query, req.companyId);
    logger.logRequest(req, 'Get all color palettes');
    return ApiResponse.paginated(res, result.data, result.pagination);
});

const getById = asyncHandler(async (req, res) => {
    const palette = await colorPaletteService.getById(req.params.id, req.companyId);
    logger.logRequest(req, 'Get color palette by ID');
    return ApiResponse.success(res, { palette });
});

const create = asyncHandler(async (req, res) => {
    const { name, primary_bg_color, primary_text_color, secondary_text_color, paragraph_color, is_active } = req.body;
    const palette = await colorPaletteService.create(
        { name, primary_bg_color, primary_text_color, secondary_text_color, paragraph_color, is_active },
        req.user.id,
        req.companyId
    );
...
const remove = asyncHandler(async (req, res) => {
    await colorPaletteService.remove(req.params.id, req.user.id, req.companyId);
