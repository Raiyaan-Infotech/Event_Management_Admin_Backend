...
const getUiBlocks = asyncHandler(async (req, res) => {
    const result = await uiBlockService.getUiBlocks(req.query, req.company?.id);
    logger.logRequest(req, 'Get all UI blocks');
    return ApiResponse.paginated(res, result.data, result.pagination);
});

const getUiBlockById = asyncHandler(async (req, res) => {
    const uiBlock = await uiBlockService.getUiBlockById(req.params.id, req.company?.id);
...
const createUiBlock = asyncHandler(async (req, res) => {
    const uiBlock = await uiBlockService.createUiBlock(req.body, req.user?.id, req.company?.id);
...
const updateUiBlock = asyncHandler(async (req, res) => {
    const uiBlock = await uiBlockService.updateUiBlock(req.params.id, req.body, req.user?.id, req.company?.id);
...
const deleteUiBlock = asyncHandler(async (req, res) => {
    await uiBlockService.deleteUiBlock(req.params.id, req.user?.id, req.company?.id);
