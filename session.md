...
### 587. Live plan limits set to 2 events / 3 guests / 3 RSVP per event — for Jamal to test the limits

Jamal asked for this. All 4 active plans on production (Basic 9, Standard 10, Premium 11, Free 12) now have `max_events=2, max_guests_per_event=3, max_rsvp_per_event=3`. Old values are backed up in `D:\Jamal\_prod_backups\prod-plan-limits-1790280229394.json` (were Basic 2/50/1, Standard 3/15/15, Premium 5/25/25, Free 1/5/5). The soft-deleted plan 8 was not changed. **Restore those values when testing is done.** Ismail (#27) is already over the guest limit (50 of 3) and at the event limit (2 of 2).

**Update (same day):** at Jamal's request the live limits were changed to tiers instead: Free 1/2/2, Basic 2/4/4, Standard 3/6/6, Premium 4/8/8 (events / guests / RSVP per event). The original values are still in the same backup file.

**Final (same day), Jamal's own numbers:** events 1/2/5/10, guests 5/10/20/50, RSVP per event 10/25/40/60, photos 5/10/20/50, videos 1/2/5/10 (Free/Basic/Standard/Premium). Storage is worked out from these because it is per ACCOUNT while photos/videos are per EVENT: events x (photos x 2 MB + videos x 5 MB), using the gallery per-file caps. That gives 15 MB, 60 MB, 325 MB -> 1 GB, 1500 MB -> 2 GB (the field only takes whole numbers 1-100 in MB/GB, so these were rounded up). Backup before this change: `prod-plan-limits-before-tiers-1790280873859.json`. The very first values are still in `prod-plan-limits-1790280229394.json`.

### 588. Test data for the plan limits — seeder written; Jamal ran it on live

`src/database/seeders/plan-limits-test-data.seeder.js` fills each client to one below its limit. Guests: Jamal +4, Arsath +19, Najeeb +49 (Ismail is already over his limit with 50). Participants on each first event: Jamal +9, Ismail +22, Arsath +39, Najeeb +59. Half of the participants are linked to a phone-book guest and half are strangers, with mixed answers. Every row is tagged `notes = 'TEST DATA §588'`. `--apply` inserts and `--remove` deletes them. It refuses to run twice. The dry run passed against production. My own attempt to insert was blocked by the permission check, so Jamal ran `--apply` himself.

**Applied on live by Jamal (2026-09-25):** guests are now Jamal 4/5, Ismail 50/10 (over), Arsath 19/20, Najeeb 49/50. Participants on each first event: Jamal 9/10, Ismail 24/25, Arsath 39/40, Najeeb 59/60. Remove them with `--remove`.

**⚠ Open: "Could not load your guests" on the Guests page.** The message is `errorHandler`'s reply to any `SequelizeDatabaseError`. Checked: the current `listGuests` works against the LIVE DB for all 4 clients with the portal's exact params (page 1, limit 8, with and without search). GitHub `main` has the new code, and every require path matches the filenames exactly. The most likely cause is a server still running pre-§581 code, which queries `event_guests` (a table that no longer exists): either a failed or pending Render deploy, or a local backend started before the rename. Waiting on Jamal to check the Render deploy status or the Network response.

### 589. App menus: Family hidden, Messages/Splash showing as tiles

- **Family missing is by design (§ presentOne).** A PARTICIPANT sees Family only if their own relationship or group is family-like (family/relative/friend …). All 3 real participants on live chose relationship "Other", so it is hidden for them. The HOST always sees it: Family is granted to every plan.
- **Messages / Splash Screens were app tiles, fixed.** They are portal sections (`PORTAL_SECTION_SLUGS`), and `presentOne` sent every granted portal section to the app, even to participants. The app has no screen for either, so they opened "coming soon". The new `NOT_APP_TILE_SLUGS` keeps them off the app tiles. The plan still grants them for the portal, and the event splash still plays on event open (`/splash-screens/for-event`, which does not read `menus`). Verified locally: the host and a participant both lost the two tiles, the host keeps Family and Guests, and a participant with no family tag does not see Family.
...
**Menus (data only, tool `set-default-menus.js`):** Default = splash-screens, event-invitation, participants, venue, rsvp, agenda, guests (and `menu_group` = core); every other menu Add-on (`addon`). Social Wall set **Inactive** — gone from every menu, row kept for a possible return; the app's Social Wall code untouched. The tool also grants each Default menu to every live plan that lacks it. **LOCAL applied** (10 menus changed, backup `local-default-menus-1790311918743.json`). **PRODUCTION applied by Jamal** (same 10 changes, 0 grants needed; backup `D:\Jamal\prod-backups\prod-default-menus-1790312427876.json`). Production has no `event-information` menu (local does). `invitation_image` column also **added on production** by Jamal, before the deploy.
